...
async fn main() -> Result<(), std::convert::Infallible> {
    ...
    #[cfg(feature = "tls")]
    let certificates = Client::certificates_from(&opt.certificates).unwrap();
    let xaynet_client = Client::new(
        &opt.url,
        #[cfg(feature = "tls")]
        certificates,
    )
...
fn generate_agent_config() -> AgentSettings {
    ...
    AgentSettings::new(keys, mask_config)
}
